...
- **모니터:** 실제 NVDA 60거래일 캔들이 배속으로 흘러간다. 한 바퀴는 약 46초이고, 끝나면 무한 반복한다.
- **매수/매도:** 매매하는 날에는 차트가 멈추고 모션이 나온다. 매수 때는 초록 설탕 방울에 주둥이를 뻗고, 매도 때는 보라 쓴맛 방울을 맛본 뒤 입을 닦는다. 모션끼리는 겹치지 않는다.
- **클럽 조명:** 천장에서 색이 계속 바뀌는 무빙 스포트라이트 4개(빛줄기 포함)가 124 BPM 비트에 맞춰 번쩍이고, 책상 아래와 의자 밑 LED, 바닥 그리드도 색이 돈다. 절망하는 순간 전부 꺼지고 주변이 새까매지며 초파리 위 스포트라이트만 남는다.
- **책상:** 에너지 드링크 캔 피라미드, 위스키·보드카 병(빈 병, 쓰러진 병 포함), 꽁초 가득한 재떨이. 초파리는 입에 담배를 물고 있고, 5.5초마다 한 모금 빨면 불씨가 밝아지고 연기를 내뿜는다. 실제 브랜드 로고는 쓰지 않았다.
- **오른쪽 아래:** 실제 뉴런 좌표 4만 개와 그날 활성화된 단맛 경로(금색), 쓴맛 경로(초록), MN9(분홍) 하이라이트가 보인다.
- **왼쪽 위:** 초파리 수익률, 존버 수익률, 최근 매매 기록.
- **엔딩 (손실일 때 16초):** 모니터에 최종 성적표가 뜨고, 초파리가 몸을 기울여 확인한 뒤 → 깜짝 놀라 → 카메라 쪽으로 돌아앉아 고개를 하늘로 젖히고 두 손으로 눈을 가린 채 약 12초간 흐느낀다 (몸을 흔들고, 몇 초마다 고개를 더 젖히며 울부짖고, 눈물이 흐른다). 방이 어두워지고 차가운 스포트라이트가 초파리를 비추며, 카메라는 천천히 다가간다.
- **화면 문구:** 전부 영어 (Fly's account, Buy & hold, Sweet/Bitter/MN9 등).
